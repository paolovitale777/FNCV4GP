...
# NV4GP logo icon (marker-grid -> prediction curve), embedded as base64 PNG
...
class MarkerFilterApp(tk.Tk):
    def __init__(self):
        ...
        self._logo_full = tk.PhotoImage(data=_NV4GP_LOGO_PNG_B64)
    ...
    def _build_home_page(self, parent):
        ...
        tk.Label(
            wordblock,
            text="Nested Validation for Genomic Prediction",
            font=("Segoe UI", 11),
            bg=NAV, fg="#90cdf4"
        ).pack(anchor="w")
                ...
                for outer_fold, (tr_idx, te_idx) in enumerate(outer_cv.split(X), start=1):
                    ...
                    if args["use_gamma"]:
                        for C, epsilon, kernel, gamma in grid:
                            ...
                            for itr_idx, iva_idx in inner_cv.split(X_train_outer):
                                X_inner_train_raw = X_train_outer.iloc[itr_idx]
                                X_inner_val_raw = X_train_outer.iloc[iva_idx]
                                inner_scaler = StandardScaler()
                                Xtr = inner_scaler.fit_transform(X_inner_train_raw)
                                Xva = inner_scaler.transform(X_inner_val_raw)
